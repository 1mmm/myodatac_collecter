...
boolean f1 = false, f2 = false, f3 = false, la = false, le = false, lo = false, lg = false, l1 = false, l2 = false, l3 = false, l4 = false, l5 = false,l11 = false, l22 = false, l33 = false, l44 = false, l55 = false;
int num = 0, dnum = 0;
...
public:
	...
	void openFiles() {
		...
		std::ostringstream dirn;
		dirn <<"md "<<userName;
		system(dirn.str().c_str());
		// Open file for EMG log
		if (emgFile.is_open()) {
			emgFile.close();
		}
		std::ostringstream emgFileString;
		emgFileString << userName <<"\\"<< No <<  "-" << gesName << "-" << times << "-emg-" << timestamp << ".txt";
		...
		gyroFileString << userName << "\\"<< No  << "-" << gesName << "-" << times << "-gyro-" << timestamp << ".txt";
		...
		accelerometerFileString << userName << "\\" << No  << "-" << gesName << "-" << times << "-accelerometer-" << timestamp << ".txt";
		...
		orientationFileString << userName << "\\" << No   << "-" << gesName << "-" << times << "-orientation-" << timestamp << ".txt";
		...
		orientationEulerFileString << userName << "\\" << No  << "-" << gesName << "-" << times << "-orientationEuler-" << timestamp << ".txt";
		...
		emgFileString2 << userName << "\\" << No  << "-" << gesName << "-" << times << "-emg-" << timestamp << ".txt";
		...
		gyroFileString2 << userName << "\\" << No  << "-" << gesName << "-" << times << "-gyro-" << timestamp << ".txt";
		...
		accelerometerFileString2 << userName << "\\" << No  << "-" << gesName << "-" << times << "-accelerometer-" << timestamp << ".txt";
		...
		orientationFileString2 << userName << "\\" << No  << "-" << gesName << "-" << times << "-orientation-" << timestamp << ".txt";
		...
		orientationEulerFileString2 << userName << "\\" << No  << "-" << gesName << "-" << times << "-orientationEuler-" << timestamp << ".txt";
	...
	void onOrientationData(myo::Myo *myo, uint64_t timestamp, const myo::Quaternion< float > &rotation) {
		...
		else
		{
			if (f3&&lo) {
				lo = false;
				if (l2) {
					orientationFile2 << std::endl;
					orientationEulerFile2 << std::endl;
					l2 = false;
				}
	...
	void onAccelerometerData(myo::Myo *myo, uint64_t timestamp, const myo::Vector3< float > &accel) {
		...
		if (identifyMyo(myo)==1)
		{
			f1 = true;
			if (f3&&!la) {
				if (l33) { accelerometerFile << std::endl; l33 = false; }
				la = true;
	...
	void onGyroscopeData(myo::Myo *myo, uint64_t timestamp, const myo::Vector3< float > &gyro) {
		...
		{   
			if (f3&&!lg) {
				if (l44) { gyroFile << std::endl; l44 = false; }
				lg = true;
